...
function renderMarkdownContent(content: string) {
            ...
            <thead>
              <tr className="border-b bg-slate-50">
                {tableRows[0].map((cell, ci) => (
                  <th key={ci} className="px-4 py-2.5 text-left font-semibold text-slate-700">{cell.trim()}</th>
                ))}
              </tr>
            </thead>
            <tbody>
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (line.startsWith('# ')) {
      elements.push(<h2 key={i} className="mb-4 mt-8 text-3xl font-extrabold tracking-tight text-slate-900">{line.slice(2)}</h2>);
    } else if (line.startsWith('## ')) {
      elements.push(<h2 key={i} className="mb-3 mt-8 text-2xl font-bold text-slate-900">{line.slice(3)}</h2>);
    } else if (line.startsWith('### ')) {
      elements.push(<h3 key={i} className="mb-2 mt-6 text-lg font-bold text-slate-900">{line.slice(4)}</h3>);
    } else if (line.startsWith('**') && line.endsWith('**')) {
      elements.push(<p key={i} className="mb-2 font-semibold text-slate-900">{line.slice(2, -2)}</p>);
    } else if (line.startsWith('- ')) {
      elements.push(
        <li key={i} className="ml-5 list-disc text-slate-600">{line.slice(2)}</li>
      );
    } else if (/^\d+\.\s/.test(line)) {
      elements.push(
        <li key={i} className="ml-5 list-decimal text-slate-600">{line.replace(/^\d+\.\s/, '')}</li>
      );
    } else if (line.startsWith('*') && line.endsWith('*') && !line.startsWith('**')) {
      elements.push(<p key={i} className="mb-4 italic text-slate-500">{line.slice(1, -1)}</p>);
    } else if (line.trim() === '') {
      elements.push(<div key={i} className="h-2" />);
    } else {
      elements.push(<p key={i} className="mb-3 leading-relaxed text-slate-600">{line}</p>);
    }
  }
  flushTable();
      ...
      <div className="container mx-auto px-4 py-10">
        <div className="flex gap-8">
          ...
          <div className="min-w-0 flex-1">
            <Card className="rounded-2xl border-slate-200 bg-white p-6 md:p-10">
              {/* Category */}
              ...
              <div className="mb-6 flex items-center gap-4 rounded-xl bg-slate-50 p-4">
                ...
                <div className="flex-1">
                  <Link href={`/pros/${post.authorId}`} className="font-semibold text-slate-900 hover:text-reno-green">
                    {post.authorName}
                  </Link>
              ...
              <div className="mt-8 flex flex-wrap gap-2">
                {post.tags.map((tag) => (
                  <Badge
                    key={tag}
                    className="rounded-full bg-slate-100 px-3 py-1 text-xs font-medium text-slate-600 border-transparent hover:bg-slate-200"
                  >
                    #{tag}
                  </Badge>
                ))}
            ...
            {/* Comments Section */}
            <Card className="mt-6 rounded-2xl border-slate-200 bg-white p-6 md:p-8">
              ...
              <div className="mb-6 rounded-xl bg-slate-50 p-4">
                <textarea
                  ...
                  rows={3}
                  className="mb-3 w-full rounded-xl border border-slate-200 bg-white px-4 py-2.5 text-sm text-slate-900 outline-none placeholder-slate-400 transition-colors focus:border-reno-green focus:ring-2 focus:ring-reno-green-light"
                />
                      ...
                      <div className="flex-1">
                        <div className="flex items-center gap-2 mb-1">
                          <Avatar className="h-6 w-6">
                            ...
                            <AvatarFallback className="bg-slate-100 text-[10px]">{comment.authorName.charAt(0)}</AvatarFallback>
                          </Avatar>
                          <span className="text-sm font-semibold text-slate-700">{comment.authorName}</span>
                          <span className="text-xs text-slate-400">
                            {new Date(comment.createdAt).toLocaleDateString('en-CA', { month: 'short', day: 'numeric' })}
                          </span>
                        </div>
                        <p className="text-sm text-slate-600">{comment.content}</p>
                        <button className="mt-1 text-xs font-medium text-slate-400 hover:text-slate-600">
                          Reply
                        </button>
                                ...
                                <div className="flex-1">
                                  <div className="flex items-center gap-2 mb-1">
                                    <Avatar className="h-5 w-5">
                                      ...
                                      <AvatarFallback className="bg-slate-100 text-[9px]">{reply.authorName.charAt(0)}</AvatarFallback>
                                    </Avatar>
                                    <span className="text-sm font-semibold text-slate-700">{reply.authorName}</span>
            ...
            {/* Related Posts */}
            <div className="mt-8">
              <h2 className="mb-4 text-lg font-bold text-slate-900">Related Articles</h2>
              <div className="grid gap-6 md:grid-cols-3">
                {relatedPosts.map((relPost) => (
                  <Card key={relPost.id} className="group overflow-hidden rounded-2xl border-slate-200 bg-white transition-all hover:shadow-md">
                    <Link href={`/blog/${relPost.id}`}>
                      ...
                      <div className="p-4">
                        ...
                        <h3 className="text-sm font-bold text-slate-900 line-clamp-2 group-hover:text-reno-green">
                          {relPost.title}
                        </h3>
                      </div>
                    </Link>
          ...
          <div className="hidden shrink-0 lg:block">
            <div className="sticky top-24 flex flex-col items-center gap-3">
              <Button variant="ghost" size="icon" className="rounded-lg text-slate-400 hover:bg-slate-100 hover:text-slate-600">
