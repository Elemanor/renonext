...
function getStatusRing(member: CrewMember): string {
  ...
  if (member.status === 'not_arrived') return 'ring-2 ring-slate-300';
...
function getStatusBg(member: CrewMember): string {
  ...
  if (member.status === 'not_arrived') return 'bg-slate-100 text-slate-600';
  if (member.status === 'on_site' && !member.insideGeofence) return 'bg-amber-100 text-amber-700';
  return 'bg-reno-green-100 text-reno-green-700';
}

function getWsibBadge(status: string): { label: string; classes: string } {
  if (status === 'valid') return { label: 'Valid', classes: 'bg-reno-green-50 text-reno-green-700' };
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h1 className="text-2xl font-bold tracking-tight text-slate-900">Crew Management</h1>
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          ...
          <CardContent className="p-5">
            <div className="flex items-center justify-between">
              <div className="rounded-xl bg-reno-green-50 p-2.5">
                <Users className="h-5 w-5 text-reno-green-600" />
              </div>
            ...
            <p className="mt-3 text-sm font-medium text-slate-500">Workers On Site Now</p>
            <p className="mt-1 text-2xl font-bold tracking-tight text-slate-900">
              {crewSummary.onSiteNow}{' '}
              <span className="text-base font-medium text-slate-400">of {crewSummary.totalWorkers}</span>
            </p>
          </CardContent>
        </Card>
        ...
        <Card className="rounded-2xl border border-slate-200 bg-white shadow-none">
          <CardContent className="p-5">
            <div className="flex items-center justify-between">
              ...
              <Badge className="rounded-full border-transparent bg-primary-50 px-2 py-0.5 text-xs font-semibold text-primary-700">
                LIVE
              </Badge>
            </div>
            <p className="mt-3 text-sm font-medium text-slate-500">Hours Today</p>
            <p className="mt-1 text-2xl font-bold tracking-tight text-slate-900">
        ...
        <Card className="rounded-2xl border border-red-200 bg-red-50/30 shadow-none">
          <CardContent className="p-5">
            ...
            <p className="mt-3 text-sm font-medium text-slate-500">Ghost Attempts Blocked</p>
        ...
        <Card className="rounded-2xl border border-slate-200 bg-white shadow-none">
          <CardContent className="p-5">
            ...
            <p className="mt-3 text-sm font-medium text-slate-500">WSIB Compliant</p>
            <p className="mt-1 text-2xl font-bold tracking-tight text-slate-900">
              {crewSummary.wsibCompliant}
              <span className="text-base font-medium text-slate-400">/{crewSummary.totalWorkers}</span>
            </p>
          </CardContent>
        </Card>
      </div>
        ...
        <CardContent className="p-5">
          <div className="flex items-start gap-3">
            ...
            <div className="min-w-0 flex-1">
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b border-amber-200">
                      ...
                      <th className="pb-2 px-4 text-right font-medium text-reno-green-700">Verified (GPS)</th>
                      <th className="pb-2 pl-4 text-right font-bold text-slate-900">You Save</th>
                    </tr>
                  </thead>
                  <tbody className="text-slate-800">
                    <tr className="border-b border-amber-100">
                      <td className="py-2.5 pr-4 font-medium text-slate-600">Workers</td>
                      <td className="py-2.5 px-4 text-right">{billingComparison.invoiceClaim.workers}</td>
                      <td className="py-2.5 px-4 text-right font-semibold text-reno-green-700">
                    ...
                    <tr className="border-b border-amber-100">
                      <td className="py-2.5 pr-4 font-medium text-slate-600">Hours</td>
                    ...
                    <tr>
                      <td className="py-2.5 pr-4 font-medium text-slate-600">Total</td>
                      ...
                      <td className="py-2.5 pl-4 text-right font-bold text-reno-green-700">
              ...
              <div className="mt-4 flex gap-2">
                <Button className="h-auto rounded-xl bg-reno-green-600 px-4 py-2 text-xs font-semibold text-white shadow-sm hover:bg-reno-green-700">
      ...
      <div className="space-y-4">
        ...
        {projects.map((project) => {
          ...
          return (
            <Card key={project.id} className="rounded-2xl border border-slate-200 bg-white shadow-sm">
              {/* Project Header */}
              <button
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <div className="flex items-center gap-2">
                      ...
                      <span className="relative flex h-2.5 w-2.5">
                        <span className="absolute inline-flex h-full w-full animate-ping rounded-full bg-reno-green-400 opacity-75" />
                        <span className="relative inline-flex h-2.5 w-2.5 rounded-full bg-reno-green-500" />
                      </span>
              ...
              {isExpanded && (
                <div className="border-t border-slate-100">
                            ...
                            >
                              ...
                              <td className="px-3 py-3">
                                <div className="flex flex-col gap-1">
                                  <Badge className="w-fit rounded-full border border-slate-200 bg-slate-50 px-2 py-0 text-[10px] font-medium text-slate-600">
                                    {member.trade}
                                  </Badge>
                              ...
                              <td className="px-3 py-3">
                                {member.clockedIn ? (
                                  <div className="flex items-center gap-1.5">
                                    <Clock className="h-3.5 w-3.5 text-reno-green-500" />
                                    <span className="font-medium text-slate-900">{member.clockedIn}</span>
                                  </div>
                              ...
                              <td className="px-3 py-3 text-right">
                                <span
                                  className={`font-semibold ${
                                    member.hoursToday > 0 ? 'text-slate-900' : 'text-slate-300'
                                  }`}
                                >
                              ...
                              <td className="px-3 py-3 text-center">
                                {member.gpsVerified && member.insideGeofence ? (
                                  <div className="flex items-center justify-center gap-1">
                                    ...
                                    <span className="text-[10px] text-reno-green-600">Verified</span>
                                  </div>
                                ) : member.gpsVerified && !member.insideGeofence ? (
                                  <div className="flex items-center justify-center gap-1">
                              ...
                              <td className="px-3 py-3 text-center">
                                {member.jsaSigned ? (
                                  <div className="flex items-center justify-center gap-1">
                                    <CheckCircle className="h-3.5 w-3.5 text-reno-green-500" />
                                    <span className="text-[10px] text-reno-green-600">Signed</span>
                                  </div>
                              ...
                              <td className="px-5 py-3 text-right">
                                <span
                                  className={`font-semibold ${
                                    costToday > 0 ? 'text-slate-900' : 'text-slate-300'
                                  }`}
                                >
                  ...
                  <div className="flex flex-wrap items-center justify-between gap-4 border-t border-slate-100 bg-slate-50/50 px-5 py-3">
                    ...
                    <div className="flex items-center gap-1.5 text-sm">
                      ...
                      <span className="text-slate-500">Project cost today:</span>
      ...
      <Card className="rounded-2xl border border-slate-200 bg-white shadow-sm">
        <CardContent className="p-6">
          <div className="flex items-start gap-4">
            ...
            <div className="min-w-0 flex-1">
              <h3 className="text-lg font-bold text-slate-900">The Anti-Subcontracting Lock</h3>
              ...
              <div className="mt-5 space-y-3">
                ...
                {[
                  ...
                  <div key={item.step} className="flex items-center gap-3">
                    <div className="flex h-6 w-6 shrink-0 items-center justify-center rounded-full bg-reno-green-100 text-xs font-bold text-reno-green-700">
                      {item.step}
                    </div>
              ...
              <div className="mt-5 rounded-xl bg-reno-green-50 px-4 py-3">
        ...
        <CardContent className="p-6">
          <div className="flex items-start gap-4">
            ...
            <div className="min-w-0 flex-1">
              ...
              <div className="mt-5 rounded-xl border border-reno-green-200 bg-reno-green-50/50 p-4">
                <h4 className="text-xs font-bold uppercase tracking-wider text-reno-green-700">
                  Today&apos;s JSA Status
                </h4>
                <div className="mt-2 flex items-center gap-2">
                  <CheckCircle className="h-5 w-5 text-reno-green-500" />
                  <span className="text-sm font-semibold text-reno-green-800">
              ...
              <div className="mt-4 flex gap-2">
                <Button
                  variant="outline"
                  className="h-auto rounded-xl border-slate-200 px-4 py-2 text-xs font-medium text-slate-700"
                >
      ...
      <Card className="rounded-2xl border border-slate-200 bg-slate-50/50 shadow-none">
        <CardContent className="p-6">
          <h3 className="mb-5 text-center text-base font-bold text-slate-900">
            What This Feature Saves You
          </h3>
          <div className="grid gap-6 sm:grid-cols-3">
            <div className="text-center">
              <div className="mx-auto flex h-12 w-12 items-center justify-center rounded-xl bg-reno-green-100">
                <DollarSign className="h-6 w-6 text-reno-green-600" />
              </div>
              <p className="mt-3 text-lg font-bold text-slate-900">10-15%</p>
              <p className="text-sm font-semibold text-slate-700">Labor Cost Savings</p>
            ...
            <div className="text-center">
              ...
              <p className="mt-3 text-lg font-bold text-slate-900">100%</p>
              <p className="text-sm font-semibold text-slate-700">JSA Compliance</p>
              <p className="mt-1 text-xs text-slate-500">
                Every worker signs before every shift
              </p>
            </div>
            <div className="text-center">
              <div className="mx-auto flex h-12 w-12 items-center justify-center rounded-xl bg-reno-purple-100">
                <Shield className="h-6 w-6 text-reno-purple-600" />
              </div>
              <p className="mt-3 text-lg font-bold text-slate-900">Zero</p>
              <p className="text-sm font-semibold text-slate-700">Liability Gaps</p>
              <p className="mt-1 text-xs text-slate-500">
