  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
        <PortfolioKpi icon={HardHat} label="Active" value={String(activeCount)} color="text-primary-600 bg-primary-50" />
        <PortfolioKpi icon={CheckCircle2} label="On Track" value={String(onTrackCount)} color="text-reno-green-600 bg-reno-green-50" />
        <PortfolioKpi icon={DollarSign} label="Total Value" value={formatCurrency(totalValue)} color="text-amber-600 bg-amber-50" />
        <PortfolioKpi icon={Target} label="Avg Completion" value={`${avgCompletion}%`} color="text-reno-purple-600 bg-reno-purple-50" />
      </div>
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        <Card>
          <CardContent className="p-5">
            <h2 className="mb-2 text-sm font-semibold text-slate-900">Project Health</h2>
        ...
        <Card>
          <CardContent className="flex items-center justify-center p-5">
            <div className="text-center">
              <p className="text-4xl font-bold tabular-nums text-slate-900">{avgCompletion}%</p>
              <p className="mt-1 text-xs text-slate-500">Average Completion</p>
                    ...
                    <div className="mt-3 flex items-center gap-4">
                      <div className="min-w-0 flex-1">
                        <div className="mb-1 flex items-center justify-between text-xs text-slate-500">
                          <span>Progress</span>
                          <span className="font-semibold tabular-nums text-slate-700">{project.percent_complete}%</span>
                        </div>
                      ...
                      <div className="shrink-0 text-right">
                        ...
                        <p className="text-sm font-semibold tabular-nums text-slate-900">{formatCurrency(project.contract_value)}</p>
                      </div>
                    </div>
                  </div>
