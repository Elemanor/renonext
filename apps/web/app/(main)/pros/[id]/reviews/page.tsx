    ...
    <div className="min-h-screen bg-slate-50">
      <div className="container mx-auto px-4 py-8">
        ...
        <div className="mb-6 flex items-center gap-2 text-sm text-slate-500">
          ...
          <Link href="/pros" className="hover:text-slate-700">Pros</Link>
          <ChevronRight className="h-3.5 w-3.5" />
          <Link href={`/pros/${resolvedParams.id}`} className="hover:text-slate-700">{pro.name}</Link>
          <ChevronRight className="h-3.5 w-3.5" />
        ...
        <div className="grid gap-8 lg:grid-cols-3">
          ...
          <div className="space-y-6">
            {/* Rating Overview */}
            <Card className="sticky top-20 rounded-2xl border-slate-200 bg-white p-6">
              <div className="mb-4 flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-sm text-slate-500">{pro.name}</p>
                </div>
              </div>
              ...
              <div className="mb-5 space-y-1.5">
                {starDistribution.map((row) => (
                  <div key={row.stars} className="flex items-center gap-2.5">
                    <span className="w-4 text-right text-xs font-semibold text-slate-600">{row.stars}</span>
                ...
                {[
                  ...
                  <div key={item.label} className="flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <span className="min-w-[2ch] text-right text-xs font-bold text-slate-700">
          ...
          <div className="space-y-4 lg:col-span-2">
            {/* Filter / Sort Bar */}
            <Card className="flex flex-wrap items-center justify-between gap-3 rounded-2xl border-slate-200 bg-white p-4">
              ...
              <Button variant="ghost" className="flex items-center gap-1 text-xs font-medium text-slate-500 h-auto px-2 py-1">
            ...
            {/* Empty State */}
            {reviews.length === 0 && (
              <Card className="rounded-2xl border-slate-200 bg-white p-12 text-center">
            ...
            {reviews.map((review: any) => (
              <Card key={review.id} className="rounded-2xl border-slate-200 bg-white p-6">
                {/* Header */}
                ...
                {/* Title + Comment */}
                <h3 className="mb-2 font-semibold text-slate-900">{review.title}</h3>
                <p className="mb-4 text-sm leading-relaxed text-slate-600">{review.comment}</p>
                ...
                {review.proResponse && (
                  <div className="mb-4 rounded-xl bg-reno-green-light/50 p-4">
                    ...
                    <p className="text-sm text-slate-600">{review.proResponse}</p>
                  </div>
                )}
                ...
                <div className="flex items-center gap-2 pt-2 border-t border-slate-100">
                  <Button
                    variant="ghost"
                    className="flex items-center gap-1.5 rounded-lg px-2 py-1 text-xs text-slate-400 transition-all duration-200 hover:bg-slate-50 hover:text-slate-600 h-auto"
                  >
