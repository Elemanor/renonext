        ...
        {teamData.map((m) => (
          <div key={m.name} className="flex flex-col items-center">
            <div className="relative">
              <div className={`w-16 h-16 rounded-full p-[2px] ${m.onSite ? 'bg-gradient-to-br from-reno-green-500 to-reno-green-500' : 'bg-gradient-to-br from-slate-600 to-slate-700'}`}>
              ...
              {m.onSite && (
                <div className="absolute -bottom-0.5 right-0 w-5 h-5 bg-[#0a0a0f] rounded-full flex items-center justify-center">
                  <CheckCircle className="w-4 h-4 text-reno-green-400" />
                </div>
              )}
              {m.onSite && (
                <div className="absolute -top-1 -right-1 bg-reno-green-500 text-white text-[8px] px-1.5 py-0.5 rounded-full font-medium">Today</div>
              )}
            </div>
            <p className="text-white/85 text-xs mt-2 text-center">{m.name}</p>
            <p className="text-slate-500 text-[10px] text-center max-w-[80px] whitespace-pre-line leading-tight mt-0.5">{m.role}</p>
          </div>
        ))}
      </div>
    </div>
