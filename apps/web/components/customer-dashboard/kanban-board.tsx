...
const columns = [
  {
    ...
    tasks: [
      { title: 'HVAC ductwork install', initials: 'DP', color: 'bg-primary-500/20 text-primary-400' },
  ...
  },
  {
    ...
    tasks: [
      ...
      { title: 'Drywall hanging', initials: 'JK', color: 'bg-reno-green-500/20 text-reno-green-400' },
    ],
  },
  {
    id: 'done',
    title: 'Done',
    dot: 'bg-reno-green-400',
    cardBg: 'bg-reno-green-500/[0.04] border-reno-green-500/10',
    tasks: [
      { title: 'Foundation inspection', initials: 'RW', color: 'bg-primary-500/20 text-primary-400' },
      { title: 'Framing complete', initials: 'DP', color: 'bg-primary-500/20 text-primary-400' },
      { title: 'Plumbing rough-in', initials: 'AL', color: 'bg-reno-green-500/20 text-reno-green-400' },
    ],
  },
] as const;
      ...
      <div className="flex items-center justify-between mb-3 lg:mb-2 flex-shrink-0">
        <div className="flex items-center gap-2">
          <div className="w-7 h-7 rounded-lg bg-primary-500/20 flex items-center justify-center">
            <KanbanSquare className="w-3.5 h-3.5 text-primary-400" />
          </div>
          <h2 className="text-white/90 text-sm tracking-tight">Task Board</h2>
        </div>
        <span className="text-slate-500 text-[10px]">{totalTasks} tasks</span>
      </div>
          ...
          <div key={col.id} className="flex flex-col min-h-0">
            ...
            <div className="flex items-center justify-between mb-2 flex-shrink-0">
              <div className="flex items-center gap-1.5">
                ...
                <span className="text-[10px] text-slate-400">{col.title}</span>
              </div>
                ...
                >
                  <p
                    className={`text-[10px] leading-tight mb-1.5 ${
                      col.id === 'done' ? 'text-slate-500 line-through' : 'text-white/80'
                    }`}
                  >
                    {task.title}
                  </p>
