    ...
    <div className="bg-[#141419] rounded-xl p-4 lg:p-3.5 border border-white/[0.06] cursor-pointer shadow-lg shadow-black/20" onClick={() => setExpanded(!expanded)}>
      <div className="flex items-center justify-between mb-3">
        <div className="flex items-center gap-2">
          <div className="w-7 h-7 rounded-lg bg-primary-500/20 flex items-center justify-center">
            <ClipboardCheck className="w-3.5 h-3.5 text-primary-400" />
          </div>
      ...
      <div className="flex items-start gap-3 bg-white/[0.02] rounded-lg p-3">
        ...
        <div className="flex-1">
          ...
          <p className="text-slate-500 text-xs mt-1">Scheduled between 1 PM - 3 PM</p>
        </div>
        <div className="flex-shrink-0">
          <span className="text-primary-400 text-[10px] bg-primary-400/10 px-2 py-0.5 rounded-md">{completedCount}/{milestoneSteps.length}</span>
        </div>
      </div>
      <AnimatePresence>
        {expanded && (
          <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} transition={{ duration: 0.3, ease: [0.25, 0.46, 0.45, 0.94] }} className="overflow-hidden">
            <div className="mt-3 pt-3 border-t border-white/[0.06]">
              <p className="text-slate-500 text-[10px] mb-2">Project Milestones</p>
              <div className="space-y-0">
                {milestoneSteps.map((step, i) => (
                  <div key={step.label} className="flex items-center gap-3 py-1.5">
                    <div className="flex flex-col items-center w-5">
                      {step.completed ? (
                        <CheckCircle2 className="w-4 h-4 text-reno-green-400" />
                      ) : step.active ? (
                      ...
                      {i < milestoneSteps.length - 1 && (
                        <div className={`w-[2px] h-3 mt-0.5 ${step.completed ? 'bg-reno-green-400/30' : 'bg-slate-700/40'}`} />
                      )}
                    </div>
