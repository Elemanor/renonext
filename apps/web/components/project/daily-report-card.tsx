    ...
    <div className="rounded-xl border border-slate-200 bg-white transition-all duration-200">
      ...
      >
        <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-lg bg-slate-100">
          <WeatherIcon className="h-5 w-5 text-slate-500" />
        </div>
        ...
        {report.client_summary && (
          <p className="hidden max-w-xs truncate text-xs text-slate-500 lg:block">
            {report.client_summary}
          </p>
        )}
        <ChevronDown
      ...
      {expanded && (
        <div className="border-t border-slate-100 p-4 pt-3">
          {/* Client summary */}
          {report.client_summary && (
            <p className="mb-3 text-sm leading-relaxed text-slate-700">{report.client_summary}</p>
          )}
          ...
          <div className="mb-3">
            ...
            <ul className="space-y-1">
              {tasks.map((t) => (
                <li key={t.task_id} className="text-xs text-slate-700">
              ...
              <div className="flex gap-2">
                {report.photos.map((url) => (
                  <button
                    ...
                    className="h-16 w-16 shrink-0 overflow-hidden rounded-lg border border-slate-200 bg-slate-100 transition-all hover:ring-2 hover:ring-primary-300"
                  >
          ...
          {materials.length > 0 && (
            <div>
              <p className="mb-1 text-xs font-semibold text-slate-500">Materials Delivered</p>
              <ul className="space-y-1">
                {materials.map((m, i) => (
