...
function DeliveryStatusBadge({ status }: { status: Delivery['status'] }) {
  const styles: Record<string, string> = {
    delivered: 'bg-reno-green-500/20 text-reno-green-400',
...
function InspectionStatusIcon({ status, result }: Pick<Inspection, 'status' | 'result'>) {
  if (status === 'completed' && result === 'passed') return <CheckCircle2 className="h-4 w-4 text-reno-green-400" />;
  ...
  return (
    <AnimatedCard className="flex min-w-0 flex-col" delay={0.1}>
      ...
      <div className="mb-4 flex items-start justify-between">
        <div>
          <p className="text-xs font-medium uppercase tracking-wider text-slate-500">Today&apos;s Weather</p>
          <div className="mt-1 flex items-baseline gap-2">
            <span className="text-3xl font-bold tabular-nums text-white/90">6°C</span>
            <span className="text-sm text-slate-400">Overcast</span>
          </div>
      ...
      <div className="mb-4 flex gap-3 overflow-x-auto pb-1">
        {hourlyForecast.map((h: HourlyForecast) => (
          <div key={h.time} className="flex shrink-0 flex-col items-center gap-1">
            ...
            <WeatherIcon icon={h.icon} className="h-4 w-4 text-slate-400" />
                ...
                <div key={step.label} className="flex items-start gap-3">
                  <div className="flex flex-col items-center">
                    <div
                      className={cn(
                        ...
                        step.status === 'completed' ? 'border-reno-green-400 bg-reno-green-400' :
                        step.status === 'current' ? 'border-reno-green-400 bg-transparent' :
                        'border-slate-600 bg-transparent'
                      )}
                    />
                    {i < milestoneSteps.length - 1 && (
                      <div className={cn('mt-1 h-5 w-px', step.status === 'completed' ? 'bg-reno-green-400/40' : 'bg-slate-700')} />
                    )}
                  </div>
                  <div className="-mt-0.5 min-w-0 flex-1">
                    <p className={cn(
                      ...
                      step.status === 'current' ? 'text-white/90' : 'text-slate-500'
                    )}>
                      {step.label}
                    </p>
                  ...
                  {step.status === 'current' && (
                    <span className="shrink-0 rounded-full bg-reno-green-500/20 px-2 py-0.5 text-[10px] font-medium text-reno-green-400">
                      In Progress
                    </span>
                  )}
                </div>
            ...
            >
              {deliveries.map((d: Delivery) => (
                <div key={d.id} className="flex items-center gap-3 rounded-xl bg-white/[0.03] p-3">
                  <Truck className={cn(
                    'h-4 w-4 shrink-0',
                    d.status === 'delivered' ? 'text-reno-green-400' :
                    d.status === 'en-route' ? 'text-amber-400' : 'text-primary-400'
                  )} />
                  ...
                  <div className="flex shrink-0 flex-col items-end gap-1">
                    ...
                    <span className="text-[10px] text-slate-600">{d.eta || d.time}</span>
                  </div>
                </div>
              ))}
            </motion.div>
              ...
              {inspections.map((ins: Inspection) => (
                <div key={ins.id} className="flex items-center gap-3 rounded-xl bg-white/[0.03] p-3">
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <p className="text-[10px] text-slate-500">{ins.inspector} · {ins.date}</p>
                  </div>
