...
const aqiLevel = 42;
const aqiLabel = 'Good';
const aqiColor = 'text-reno-green-400';
  ...
  return (
    <AnimatedCard delay={0.35}>
      ...
      <div className="mb-3 flex items-center gap-2">
        <ShieldCheck className="h-4 w-4 text-reno-green-400" />
      ...
      <div className="mb-3 flex items-baseline gap-2">
        ...
        <span className="text-xs text-slate-400">AQI — {aqiLabel}</span>
      </div>
      ...
      <div className="mb-3 h-1.5 overflow-hidden rounded-full bg-white/[0.06]">
        <motion.div
          ...
          className="h-full rounded-full bg-gradient-to-r from-reno-green-500 to-reno-green-400"
        />
      </div>
      ...
      >
        <span className="text-[11px] text-slate-400">Safety Checklist</span>
            ...
            <div className="mt-2 space-y-1.5">
              {safetyChecks.map((check, i) => (
                <div key={i} className="flex items-center gap-2">
                  {check.checked ? (
                    <CheckCircle2 className="h-3.5 w-3.5 shrink-0 text-reno-green-400" />
                  ) : (
                    <Circle className="h-3.5 w-3.5 shrink-0 text-slate-600" />
                  )}
                  <span className={cn(
                    'text-[11px]',
                    check.checked ? 'text-slate-400' : 'text-slate-500'
                  )}>
                    {check.label}
                  </span>
                </div>
