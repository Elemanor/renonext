      ...
      {nearbyCities.map((city) => {
        ...
        return (
          <Link
            ...
            className="group flex flex-col rounded-xl border border-gray-200 bg-white p-4 transition-all duration-200 hover:border-reno-green/30 hover:shadow-md"
          >
            <div className="flex items-center gap-1.5">
              <MapPin className="h-3.5 w-3.5 text-gray-400" />
              <span className="text-sm font-semibold text-gray-900">{city.name}</span>
            </div>
            <p className="mt-0.5 text-[11px] text-gray-400">{city.region}</p>
            <p className="mt-2 text-sm font-bold tabular-nums text-gray-800">
              {formatPriceRange(min, max)}
            </p>
            <p className="text-[10px] text-gray-500">{baseRange.unit}</p>
            <span
