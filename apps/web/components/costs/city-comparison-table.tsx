      ...
      <table className="w-full text-sm">
        <thead>
          <tr className="border-b border-gray-100 bg-gray-50/80">
            ...
            <th className="hidden px-4 py-3 text-right font-semibold text-gray-900 sm:table-cell">vs Toronto</th>
            <th className="px-4 py-3 text-right font-semibold text-gray-900">Details</th>
          </tr>
        </thead>
        <tbody>
          {rows.map(({ city, min, max }, i) => {
            ...
            return (
              <tr key={city.slug} className={i % 2 === 0 ? 'bg-white' : 'bg-gray-50/40'}>
                <td className="px-4 py-3">
                  <div className="font-medium text-gray-800">{city.name}</div>
                  <div className="text-xs text-gray-400">{city.region}</div>
                </td>
                <td className="px-4 py-3 text-right tabular-nums text-gray-700">{formatPrice(min)}</td>
                <td className="px-4 py-3 text-right tabular-nums text-gray-700">{formatPrice(max)}</td>
