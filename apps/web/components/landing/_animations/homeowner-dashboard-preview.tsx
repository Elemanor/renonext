...
function TodayBlock() {
  const crew = [
    { name: 'Marcus T.', trade: 'Plumbing', status: 'On site', color: 'bg-reno-green-100 text-reno-green-700' },
    { name: 'Sarah K.', trade: 'Electrical', status: 'On site', color: 'bg-reno-green-100 text-reno-green-700' },
  ...
  return (
    <m.div variants={stagger} initial="hidden" animate="show" className="p-3">
      <m.div variants={fadeUp} className="mb-2 flex items-center justify-between">
        <span className="text-[10px] font-bold text-slate-800">Crew at your home</span>
      ...
      {crew.map((c) => (
        <m.div key={c.name} variants={fadeUp} className="mb-1.5 flex items-center justify-between rounded-lg border border-slate-200/80 bg-white px-2.5 py-1.5">
          <div className="flex items-center gap-2">
            ...
            <div>
              <p className="text-[8px] font-semibold text-slate-800">{c.name}</p>
              <p className="text-[6px] text-slate-400">{c.trade}</p>
            </div>
          </div>
...
function WeekBlock() {
  const days = [
    { day: 'Mon', label: 'Plumbing rough-in', border: 'border-l-emerald-500', bg: 'bg-reno-green-50/60' },
    { day: 'Tue', label: 'Electrical + framing', border: 'border-l-emerald-500', bg: 'bg-reno-green-50/60' },
  ...
  return (
    <m.div variants={stagger} initial="hidden" animate="show" className="p-3">
      <m.div variants={fadeUp} className="mb-2 flex items-center justify-between">
        <span className="text-[10px] font-bold text-slate-800">Week ahead</span>
        <span className="text-[8px] text-slate-400">Mar 10–14</span>
      </m.div>

      {days.map((d) => (
        <m.div key={d.day} variants={fadeUp} className={`mb-1 flex items-center gap-2 rounded-md border-l-2 ${d.border} ${d.bg} px-2 py-1.5`}>
          <span className="w-6 text-[7px] font-bold text-slate-500">{d.day}</span>
          <span className="text-[7px] text-slate-700">{d.label}</span>
        </m.div>
      ))}
    </m.div>
  );
...
function InspectionsBlock() {
  return (
    <m.div variants={stagger} initial="hidden" animate="show" className="p-3">
      <m.div variants={fadeUp} className="mb-2 flex items-center justify-between">
        <span className="text-[10px] font-bold text-slate-800">Next inspection</span>
      ...
      <m.div variants={fadeUp} className="rounded-lg border border-teal-200 bg-teal-50/60 p-2.5">
        ...
        <p className="mt-0.5 text-[7px] text-slate-500">City of Portland · Inspector TBD</p>
      </m.div>

      <m.div variants={fadeUp} className="mt-2">
        <p className="mb-1 text-[8px] font-medium text-slate-600">Prerequisites</p>
        {[
          ...
          <div key={item.text} className="mb-0.5 flex items-center gap-1.5 text-[7px]">
            ...
            <span className={item.done ? 'text-slate-400' : 'font-medium text-slate-700'}>{item.text}</span>
          </div>
        ))}
      </m.div>
    </m.div>
...
function StatsBlock() {
  const stats = [
    { label: 'Safety days', value: '14', sub: 'No incidents', Icon: Shield, color: 'bg-reno-green-100 text-reno-green-600' },
    ...
    { label: 'Budget used', value: '38%', sub: '$15,200 / $40,000', Icon: DollarSign, color: 'bg-primary-100 text-primary-600' },
  ];

  return (
    <m.div variants={stagger} initial="hidden" animate="show" className="p-3">
      <m.div variants={fadeUp} className="mb-2 flex items-center justify-between">
        <span className="text-[10px] font-bold text-slate-800">Project overview</span>
        <span className="text-[8px] text-slate-400">Updated today</span>
      </m.div>

      <div className="grid grid-cols-2 gap-1.5">
        {stats.map((s) => (
          <m.div key={s.label} variants={fadeUp} className="rounded-lg border border-slate-200/80 bg-white p-2">
            ...
            <p className="text-[6px] text-slate-400">{s.sub}</p>
          </m.div>
        ))}
      </div>
    </m.div>
      ...
      <div className="overflow-hidden rounded-2xl border border-slate-200/60 bg-white shadow-xl">
        <div className="border-b border-slate-200 bg-white px-4 py-2">
          <div className="flex items-center gap-2">
            ...
            <span className="ml-3 text-xs text-slate-400">renonext.com/dashboard/my-project</span>
          </div>
        </div>
        ...
        <div className="border-b border-slate-200 bg-white px-4 py-2">
          <div className="flex items-center gap-2">
            ...
            <div className="ml-3 flex flex-1 items-center gap-1 rounded-md bg-slate-100 px-2 py-0.5">
              ...
              <span className="text-[8px] text-slate-400">renonext.com/dashboard/my-project</span>
            </div>
          </div>
        </div>

        {/* Dashboard header */}
        <div className="border-b border-slate-100 bg-gradient-to-r from-teal-800 to-slate-900 px-4 py-2.5">
