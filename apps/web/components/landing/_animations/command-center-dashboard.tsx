  ...
  return (
    <m.div variants={stagger} initial="hidden" animate="show" className="p-3">
      <m.div variants={fadeUp} className="mb-2 flex items-center justify-between">
        <span className="text-[10px] font-bold text-slate-800">Next workday</span>
        <span className="text-[8px] text-slate-400">Wed, Mar 11</span>
      </m.div>

      <m.div variants={fadeUp} className="mb-2 rounded-lg border border-primary-200 bg-primary-50/60 p-2.5">
        <div className="flex items-center justify-between">
          <p className="text-[10px] font-semibold text-primary-800">Membrane + Inspector</p>
          <span className="text-[8px] tabular-nums text-primary-600">08:00–15:00</span>
        </div>
        <div className="mt-1.5 flex items-center gap-3 text-[8px] text-slate-500">
          ...
          <span className="flex items-center gap-0.5 text-reno-green-600"><Eye className="h-2.5 w-2.5" />Inspection</span>
        </div>
      </m.div>
      ...
      <m.div variants={fadeUp} className="mt-2 grid grid-cols-5 gap-1">
        {['Mon', 'Tue', 'Wed', 'Thu', 'Fri'].map((d, i) => (
          <div key={d} className={`rounded-md py-1 text-center text-[6px] font-medium ${i === 2 ? 'bg-primary-500 text-white' : 'bg-slate-100 text-slate-500'}`}>
            {d}
          </div>
        ))}
      </m.div>
...
function ReportsBlock() {
  return (
    <m.div variants={stagger} initial="hidden" animate="show" className="p-3">
      <m.div variants={fadeUp} className="mb-2 flex items-center justify-between">
        <span className="text-[10px] font-bold text-slate-800">Latest report</span>
        <span className="text-[8px] text-slate-400">Today, 4:15 PM</span>
      </m.div>

      <m.div variants={fadeUp} className="rounded-lg border border-slate-200 bg-white p-2.5">
        <div className="mb-1.5 flex items-center gap-2">
          <div className="flex h-6 w-6 items-center justify-center rounded-md bg-reno-green-100">
            <CheckCircle className="h-3 w-3 text-reno-green-600" />
          </div>
          <div>
            <p className="text-[9px] font-semibold text-slate-800">Drainage board — 60%</p>
            <p className="text-[7px] text-slate-500">3 workers · 1 photo</p>
          </div>
        </div>
      ...
      <m.div variants={fadeUp} className="mt-2 flex items-center gap-3 text-[7px] text-slate-400">
...
function ProgressBlock() {
  return (
    <m.div variants={stagger} initial="hidden" animate="show" className="p-3">
      <m.div variants={fadeUp} className="mb-2 flex items-center justify-between">
        <span className="text-[10px] font-bold text-slate-800">Stage 2 active</span>
        <span className="rounded-full bg-primary-100 px-1.5 py-0.5 text-[7px] font-bold text-primary-700">45%</span>
      </m.div>
      ...
      {[
        ...
        <m.div key={s.label} variants={fadeUp} className="mb-1.5">
          <div className="flex items-center gap-1.5 text-[7px]">
            {s.status === 'done' ? (
              <CheckCircle className="h-2.5 w-2.5 text-reno-green-500" />
            ) : s.status === 'active' ? (
              <Loader2 className="h-2.5 w-2.5 text-primary-500" />
            ) : (
              <Circle className="h-2.5 w-2.5 text-slate-300" />
            )}
            <span className={s.status === 'done' ? 'text-slate-400' : 'font-medium text-slate-700'}>{s.label}</span>
            <span className="ml-auto tabular-nums text-slate-400">{s.pct}%</span>
          </div>
          <div className="ml-4 mt-0.5 h-1 overflow-hidden rounded-full bg-slate-100">
            <div
              className={`h-full rounded-full ${s.status === 'done' ? 'bg-reno-green-500' : s.status === 'active' ? 'bg-primary-500' : 'bg-slate-200'}`}
              style={{ width: `${s.pct}%` }}
            />
          </div>
        </m.div>
      ...
      <m.div variants={fadeUp} className="mt-2 rounded-md border border-reno-green-100 bg-reno-green-50 px-2 py-1.5">
        <div className="flex items-center gap-1">
          <CheckCircle className="h-2.5 w-2.5 text-reno-green-500" />
          <span className="text-[7px] font-medium text-reno-green-700">Schedule confidence: 88%</span>
        </div>
      </m.div>
    </m.div>
  );
...
function AlertsBlock() {
  return (
    <m.div variants={stagger} initial="hidden" animate="show" className="p-3">
      <m.div variants={fadeUp} className="mb-2 flex items-center justify-between">
        <span className="text-[10px] font-bold text-slate-800">Action needed</span>
      ...
      <m.div variants={fadeUp} className="mb-1.5 rounded-lg border border-red-200 bg-red-50/60 p-2">
        <div className="flex items-center gap-1.5">
          ...
          <div>
            <p className="text-[8px] font-semibold text-slate-800">Decision: Board upgrade</p>
            <p className="text-[6px] text-slate-500">+$450 · Exceeds code requirements</p>
          </div>
        </div>
      </m.div>
      ...
      <m.div variants={fadeUp} className="mb-1.5 rounded-lg border border-amber-200 bg-amber-50/60 p-2">
        <div className="flex items-center gap-1.5">
          ...
          <div>
            <p className="text-[8px] font-semibold text-slate-800">Vibration Thu 8–4</p>
            <p className="text-[6px] text-slate-500">Drilling for drainage board</p>
          </div>
        </div>
      </m.div>

      <m.div variants={fadeUp} className="rounded-lg border border-primary-200 bg-primary-50/60 p-2">
        <div className="flex items-center gap-1.5">
          <div className="flex h-5 w-5 items-center justify-center rounded bg-primary-500">
            <Bell className="h-2.5 w-2.5 text-white" />
          </div>
          <div>
            <p className="text-[8px] font-semibold text-slate-800">Payment released: $5,550</p>
            <p className="text-[6px] text-slate-500">Stage 1 milestone</p>
          </div>
        </div>
      </m.div>
    </m.div>
      ...
      <div className="overflow-hidden rounded-2xl border border-slate-200/60 bg-white shadow-xl">
        <div className="border-b border-slate-200 bg-white px-4 py-2">
          <div className="flex items-center gap-2">
            ...
            <span className="ml-3 text-xs text-slate-400">renonext.com/dashboard/projects</span>
          </div>
        </div>
        ...
        <div className="border-b border-slate-200 bg-white px-4 py-2">
          <div className="flex items-center gap-2">
            ...
            <div className="ml-3 flex flex-1 items-center gap-1 rounded-md bg-slate-100 px-2 py-0.5">
              ...
              <span className="text-[8px] text-slate-400">renonext.com/dashboard/projects</span>
            </div>
          </div>
        </div>

        {/* Dashboard header */}
        <div className="border-b border-slate-100 bg-gradient-to-r from-slate-800 to-slate-900 px-4 py-2.5">
          ...
          {TABS.map((tab, i) => (
            ...
            >
              ...
              {i === activeIndex && !hovered && (
                <m.div
                  key={`bar-${resetKey}-${i}`}
                  className="absolute inset-x-0 bottom-0 h-[2px] rounded-full bg-primary-400/40"
                  initial={{ scaleX: 0 }}
                  animate={{ scaleX: 1 }}
